        ...
        >
          <div className="grid lg:grid-cols-[240px_1fr]">
            ...
            <div className="border-r border-sx-border p-5 space-y-5 hidden lg:block">
              <div>
                <label className="text-[10px] font-mono text-sx-text-dim uppercase tracking-wider block mb-2">
                  Instrument
                </label>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 rounded-md border border-sx-border px-3 py-2 text-[11px] font-mono text-sx-text-muted">
            ...
            <div className="flex flex-col">
              <div className="flex items-center justify-between px-5 py-3 border-b border-sx-border">
                <div className="flex items-center gap-3">
                  <span className="text-[13px] font-medium text-sx-text">Equity Curve</span>
                  <span className="text-[11px] font-mono text-sx-accent flex items-center gap-1">
                    <TrendingUp className="h-3 w-3" />
                    +24.7%
                  </span>
                </div>
                <div className="hidden md:flex gap-4 text-[10px] font-mono text-sx-text-dim">
              ...
              <div className="border-t border-sx-border">
                ...
                {fakeTrades.map((t, i) => (
                  <div
                    key={t.id}
                    className={`flex items-center justify-between px-5 py-2 text-[11px] font-mono ${i % 2 === 0 ? "bg-sx-surface/30" : ""}`}
                  >
                    <span className="text-sx-text-dim w-8">#{t.id}</span>
                    <span className={t.side === "BUY" ? "text-sx-accent" : "text-sx-red"}>{t.side}</span>
                    <span className="text-sx-text-muted flex-1 text-right ml-4">{t.entry}</span>
                    <span className="text-sx-text-muted flex-1 text-right">{t.exit}</span>
                    <span className={`w-14 text-right font-medium ${t.result === "win" ? "text-sx-accent" : "text-sx-red"}`}>
                      {t.pnl}
                    </span>
                  </div>
                ))}
